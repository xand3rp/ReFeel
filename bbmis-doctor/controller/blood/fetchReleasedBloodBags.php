<?php
	...
	$viewreleasedbloodbags = mysqli_query($connections, "
		SELECT strBloodBagSerialNo, TIMESTAMPDIFF(minute,dtmDispatchmentTimer,NOW()) as timer
		FROM tblbloodbag
		WHERE intBloodDispatchmentId = 2
		...
		while ($row = mysqli_fetch_assoc($viewreleasedbloodbags)){
			...
			if($timer < 20){
				$output .= "
					<tbody>
						<tr>
							...
							<td>$timer minutes</td>
							<td>
								<button type='button' class='btn btn_return' data-id=$serialno>
				...
				$output .= "
					<tbody>
						<tr>
							...
							<td>
								<button type='btn' class='btn mr-2 btn_discard' data-id=$serialno>
									<i class='fas fa-archive'></i>
									Discard
								</button>
								<button type='button' class='btn btn_return' data-id=$serialno disabled>
									<i class='fas fa-arrow-left'></i>
									Return
								</button>
							</td>
				...
				mysqli_query($connections, "
					UPDATE tblbloodbag
					SET stfIsBloodBagExpired = 'Yes', stfTransfusionSuccess = 'Yes'
