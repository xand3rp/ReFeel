...
if($_POST["storage_id"]){
  ...
  $fetchbloodbagtypeb = mysqli_query($connections, " SELECT strBloodBagSerialNo, dtmDateStored, stfBloodType, stfBloodTypeRhesus, intBloodVolume, intPreservativeLifespan, TIMESTAMPDIFF(DAY, NOW(), DATE_ADD(dtmDateStored, INTERVAL intPreservativeLifespan DAY)) AS 'Days Remaining' FROM tblbloodbag tbb
    ...
    WHERE tbb.intStorageId = $storage_id AND tbt.stfBloodType = 'B' AND tbb.intBloodDispatchmentId = 1 AND tbb.stfIsBloodBagExpired = 'No' AND tbb.stfIsBloodBagDiscarded = 'No' AND DATEDIFF(NOW(), tbb.dtmDateStored) <= tp.intPreservativeLifespan ORDER BY 7 ASC");
  $fetchbloodbagrotten = mysqli_query($connections, " SELECT * FROM tblbloodbag tbb
        ...
        JOIN tblbloodvolume tbv ON tbb.intBloodVolumeId = tbv.intBloodVolumeId
        JOIN tblpreservatives tp ON tbb.intPreservativeId = tp.intPreservativeId
        WHERE tbb.intStorageId = $storage_id AND tbt.stfBloodType = 'B' AND DATEDIFF(NOW(), tbb.dtmDateStored) > tp.intPreservativeLifespan
        ORDER BY tbb.dtmDateStored ASC ");
    //count query for blood BAGS
  $countbloodbagtypeb = mysqli_query($connections, " SELECT COUNT(strBloodBagSerialNo) as total FROM tblbloodbag tbb
  JOIN tblbloodtype tbt ON tbb.intBloodTypeId = tbt.intBloodTypeId
  JOIN tblstorage ts ON tbb.intStorageId = ts.intStorageId
  JOIN tblpreservatives tp ON tbb.intPreservativeId = tp.intPreservativeId
  WHERE tbb.intStorageId = $storage_id AND tbt.stfBloodType = 'B' AND tbb.intBloodDispatchmentId = 1 AND stfIsBloodBagExpired = 'No' ");
    $row = mysqli_fetch_assoc($countbloodbagtypeb);
    $total = $row["total"];
    $output =
    "
    <h4>Blood Type B</h4>
    ...
    if(mysqli_num_rows($fetchbloodbagtypeb) > 0 ) {
      while($row = mysqli_fetch_assoc($fetchbloodbagtypeb)){
        ...
        $fetchbloodbagcritical = mysqli_query($connections, " SELECT strBloodBagSerialNo, dtmDateStored, stfBloodType, stfBloodTypeRhesus, intBloodVolume, intPreservativeLifespan, TIMESTAMPDIFF(DAY, NOW(), DATE_ADD(dtmDateStored, INTERVAL intPreservativeLifespan DAY)) AS 'Days Remaining' FROM tblbloodbag tbb
          ...
          JOIN tblbloodvolume tbv ON tbb.intBloodVolumeId = tbv.intBloodVolumeId
          JOIN tblpreservatives tp ON tbb.intPreservativeId = tp.intPreservativeId
          WHERE tbb.intStorageId = $storage_id AND tbt.stfBloodType = 'B' AND DATEDIFF(NOW(), tbb.dtmDateStored) >= tp.intPreservativeCriticalPercentage AND DATEDIFF(NOW(), tbb.dtmDateStored) <= tp.intPreservativeLifespan
        ...
        $fetchbloodbagmedium = mysqli_query($connections, " SELECT strBloodBagSerialNo, dtmDateStored, stfBloodType, stfBloodTypeRhesus, intBloodVolume, TIMESTAMPDIFF(DAY, NOW(), DATE_ADD(dtmDateStored, INTERVAL intPreservativeLifespan DAY)) AS 'Days Remaining' FROM tblbloodbag tbb
          ...
          JOIN tblbloodvolume tbv ON tbb.intBloodVolumeId = tbv.intBloodVolumeId
          JOIN tblpreservatives tp ON tbb.intPreservativeId = tp.intPreservativeId
          WHERE tbb.intStorageId = $storage_id AND tbt.stfBloodType = 'B' AND DATEDIFF(NOW(), tbb.dtmDateStored) < tp.intPreservativeCriticalPercentage AND DATEDIFF(NOW(), tbb.dtmDateStored) > tp.intPreservativeFreshPercentage
          AND strBloodBagSerialNo = '$serialno'
          ORDER BY tbb.dtmDateStored ASC ");
        $fetchbloodbagfresh = mysqli_query($connections, " SELECT strBloodBagSerialNo, dtmDateStored, stfBloodType, stfBloodTypeRhesus, intBloodVolume, TIMESTAMPDIFF(DAY, NOW(), DATE_ADD(dtmDateStored, INTERVAL intPreservativeLifespan DAY)) AS 'Days Remaining' FROM tblbloodbag tbb
          ...
          JOIN tblbloodvolume tbv ON tbb.intBloodVolumeId = tbv.intBloodVolumeId
          JOIN tblpreservatives tp ON tbb.intPreservativeId = tp.intPreservativeId
          WHERE tbb.intStorageId = $storage_id AND tbt.stfBloodType = 'B' AND DATEDIFF(NOW(), tbb.dtmDateStored) <= tp.intPreservativeFreshPercentage AND DATEDIFF(NOW(), tbb.dtmDateStored) >= 0
          AND strBloodBagSerialNo = '$serialno'
          ORDER BY tbb.dtmDateStored ASC ");
