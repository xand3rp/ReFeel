<?php
	...
	$varRangeFrom = $_GET['from_range'];
	$varRangeTo = $_GET['to_range'];
	...
	$pdf -> Image('../public/img/logo-a1.png', 0.35, 0.35, 1.75);

	$pdf -> SetFont('Arial', 'B', 20);
	$pdf -> Text(2.25, 0.95, 'Reports');
	$pdf -> SetFont('Times', '', 12);
	$pdf -> Text(2.25, 1.15, date_format(date_create($varRangeFrom), 'F d, Y') . ' - ' . date_format(date_create($varRangeTo), 'F d, Y'));
	...
	$qryFetchEmpInfo = mysqli_query($connections, "
		...
		FROM tblemployee e
		WHERE e.intEmployeeId = $varEmpId
	");
	...
	$reports_remaining = mysqli_query($connections,  "
	...
	//Wasted Blood Bag per Blood Type
	$pdf -> SetFont('Times', 'B', 12);
	$pdf -> SetTextColor(255, 255, 255);
	$pdf -> SetFillColor(220, 53, 69);
	$pdf -> SetX(0.5);
	...
	while($rowBbWst = mysqli_fetch_assoc($reports_wastage))	{
		...
		$varWstCount = $rowBbWst["count_wastage"];
		$pdf -> SetFont('Times', '', 12);
		$pdf -> SetX(0.5);
	...
	$daily_count = mysqli_query($connections, "
		SELECT bt.stfBloodType, COUNT(bb.intBloodBagId) AS 'Daily Count'
		FROM tblbloodbag bb
		JOIN tblbloodtype bt ON bb.intBloodTypeId = bt.intBloodTypeId
		WHERE bb.dtmDateStored BETWEEN '$varRangeFrom' AND '$varRangeTo'
		GROUP BY 1
	");
	...
	$weekly_count = mysqli_query($connections, "
	...
	$monthly_count = mysqli_query($connections, "
	...
	$yearly_count = mysqli_query($connections, "
	...
	$bcfail_count = mysqli_query($connections, "
		...
		AND ins.dtmDateScreened BETWEEN '$varRangeFrom' AND '$varRangeTo'
		GROUP BY 1
	");
	...
	if(!isset($varBcFail))	{
		...
		$pdf -> SetTextColor(0, 0, 0);
		$pdf -> SetX(0.5);
		$pdf -> Cell(7.5, 0.3, 'No failed blood components count.', 1, 1, 'C');
	}
	$pdf -> Ln(0.25);
	...
	$bcfail_list = mysqli_query($connections, "
	...
	$difail_count = mysqli_query($connections, "
	...
	$difail_list = mysqli_query($connections, "
	...
	while($rowDiFailList = mysqli_fetch_assoc($difail_list))	{
		$pdf -> SetFont('Times', '', 12);
		$varDiFail2 = $rowDiFailList["strDisease"];
	...
	$clientsex_count = mysqli_query($connections, "
	...
	$pdf -> SetFillColor(220, 53, 69);
	$pdf -> SetX(0.5);
	$pdf -> Cell(7.5, 0.3, 'Client Count per Age', 1, 1, 'C', true);
	$pdf -> SetX(0.5);
	$pdf -> SetTextColor(0, 0, 0);
	$pdf -> Cell(3.75, 0.3, 'Age', 1, 0, 'C');
	$pdf -> Cell(3.75, 0.3, 'Count', 'TRB', 1, 'C');

	$clientage_count = mysqli_query($connections, "
